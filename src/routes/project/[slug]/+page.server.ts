...
export const actions = {
	uploadFileURLs: async (event) => {
		...
		if (projectInfo.userID !== event.locals.user.id) {
			return { uploads: [] };
		}
		...
		const uploads = await Promise.all(
			photoPayloads.map(async (payload) => {
				...
				const thumbURL = await createPresignedUploadURL(
					`photos/${projectID}/${payload.filename}/thumb`
				);
				const fullURL = await createPresignedUploadURL(
					`photos/${projectID}/${payload.filename}/full`
				);
