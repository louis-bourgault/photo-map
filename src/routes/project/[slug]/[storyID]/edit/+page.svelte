<script lang="ts">
	...
	//i dont think we actually need the full size ones now
	...
	function getPhotoById(id: number | null) {
		...
		return data.processedPhotos.find((p) => p.id === id) ?? photos.find((p) => p.id === id);
	}
...
<div class="h-[calc(100dvh-4rem)] overflow-y-auto p-4 sm:p-6">
	<div class="mx-auto flex w-full max-w-3xl flex-col gap-4">
		<div class="flex flex-wrap items-center justify-between gap-3">
			<a
				href={`/project/${page.params.slug}`}
				class="text-sm text-muted-foreground hover:text-foreground">← Back to project</a
			>
			<Button onclick={saveStory} disabled={saving} class="min-w-[140px]"
				...
				{#if item.itemType === 'photo'}
					...
					<Card.Content>
						{#if getPhotoById(item.photo) && item.photo}
							<img
								src={getPhotoById(item.photo)?.fullsizeUrl ??
									getPhotoById(item.photo)?.thumbnailUrl}
								alt="a thing"
						...
						<Dialog.Root>
							<Dialog.Trigger>
								<Button
									...
									size="sm"
									onclick={() => {
										selectedChangePhotoID = item.photo;
									}}>{item.photo ? 'Change' : 'Add'} Photo</Button
								>
							</Dialog.Trigger>
								...
								<div class="grid grid-cols-3 gap-4 py-4">
									{#each photos as photo}
										<button
											...
											class="rounded-md border-2 p-1 hover:border-primary {selectedChangePhotoID ===
											photo.id
												? 'border-primary'
												: ''}"
										>
											<img src={photo.thumbnailUrl} alt={photo.filename} class="cursor-pointer" />
										</button>
									{/each}
								</div>
								<Dialog.Footer>
						...
						<Input bind:value={item.photoCaption} placeholder="Photo caption" id="photoCaption" />
					</Card.Content>
