...
import { story, storyItem, project } from '$lib/server/db/schema.js';
...
export async function POST({ request, params, locals }) {
	...
	const [storyInfo, projectInfo] = await Promise.all([
		db
			.select()
			.from(story)
    ...
    if (projectInfo.userID !== locals.user.id) {
        error(403, 'Forbidden');
    }
	...
	const idsToDelete = oldData.filter((block) => !newIds.has(block.id)).map((block) => block.id);
